...
static char omatrix[NUM_LINES][NUM_CHAR_LINE_8];
...
static void
send_line( const char *text )
{
	const char *p;
	i2c_cmd_handle_t cmd;

	for( p = text; p < text + NUM_CHAR_LINE_8; ++p )
	{
		cmd = i2c_cmd_link_create();
		i2c_master_start(cmd);
		i2c_master_write_byte(cmd, (OLED_I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true);
...
scroll_line( void )
{
	...
	memmove( omatrix[0], omatrix[1], sizeof(omatrix) - NUM_CHAR_LINE_8 );
	for( line = 0; line < NUM_LINES-1; ++line )
	{
		set_line( line );
		send_line( omatrix[line] );
	}
}

static void
normalize_text( const char *text, int line )
{
	int len;

	memset( omatrix[line], ' ', NUM_CHAR_LINE_8 );
	if( ( len = strlen(text) ) > NUM_CHAR_LINE_8 )
		len = NUM_CHAR_LINE_8;
	memcpy( omatrix[line], text, len );
}
...
oled_text( const char *text )
{
	...
	normalize_text( text, line_num );

	set_line( line_num );
	send_line( omatrix[line_num] );
	++line_num;
}
